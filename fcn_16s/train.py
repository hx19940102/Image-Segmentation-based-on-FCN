...
fcn16s_parameters_dir = '/home/kris/PycharmProjects/eecs545/fcn_16s/fcn16s.ckpt'
...
logits, fcn_32s_variables = model.fcn_16s(images, num_classes, is_training=True)
...
# Most of parameters could be intialized with pre-trained parameters of fcn_32s model
init = slim.assign_from_checkpoint_fn(fcn32s_parameters_dir, fcn_32s_variables)
...
with tf.Session() as sess:
    ...
    saver.restore(sess, fcn16s_parameters_dir)
    ...
    for i in range(training_rounds):
        ...
        if i % 199 == 0:
            saver.save(sess, fcn16s_parameters_dir)
    coord.request_stop()
    coord.join(threads)
