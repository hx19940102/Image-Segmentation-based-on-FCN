...
res_parameters_dir = '/home/kris/PycharmProjects/eecs545/res_v1_101_8s/res_v1_101_8s.ckpt'
...
round_size = tf.round(tf.to_float(input_size[1:3]) / 8) * 8
resized_image = tf.image.resize_images(image, tf.to_int32(round_size))

# Predict the segmentation
logits, _ = model.res_101_fcn_8s(resized_image, num_classes, is_training=False)
...
with tf.Session() as sess:
    ...
    cv2.imshow('img', img)
    cv2.imshow('res', res)
    cv2.imwrite('/home/kris/PycharmProjects/eecs545/res_v1_101_8s/res.jpg', res)
    cv2.waitKey(0)
